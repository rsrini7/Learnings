...
### OpenClaw Prompts
Source : [OC.md](https://gist.github.com/mberman84/63163d6839053fbf15091238e5ada5c2#file-oc-md)
...
**Related:**- [OpenClaw(Moltbot-or-Clawdbot)-Architecture](OpenClaw%28Moltbot-or-Clawdbot)-Architecture.md) — Defines the gateway, MEMORY.md, lane queue, and cron scheduling that the workflows here (Fathom pipeline, morning briefings, three councils) rely on.- [Securing-OpenClaw-Setup](Securing-OpenClaw-Setup.md) — Required secure deployment baseline — Tailscale-isolated VPS, sandboxed Gmail/Drive accounts, and prompt-injection defenses are prerequisites for these workflows.- [OpenClaw-Whitepaper](OpenClaw-Whitepaper.md) — Broader architectural and security context for the OpenClaw capabilities (cron jobs, memory files, skills) that these video walkthroughs configure.- [claw-ecosystem](claw-ecosystem.md) — Ecosystem context — these workflows depend on OpenClaw's high-trust, broad-access model that the survey flags as Level 1 application-level isolation.
