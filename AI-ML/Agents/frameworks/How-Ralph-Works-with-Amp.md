...
**Related:**
- [Claude-Ralph-Loop](Claude-Ralph-Loop.md) — the canonical minimal bash-while-Claude-Code Ralph pattern that the Amp ralph.sh here parallels, including its pitfalls and when-not-to-use list.
- [ClaudeRalphLoop-and-AgentRalphMode](ClaudeRalphLoop-and-AgentRalphMode.md) — third Ralph implementation variant — this Amp-driven loop vs the LangChain Deep Agents Ralph Mode comparison — useful for choosing between Amp-CLI and Deep Agents runtimes.
- [AI-Coding-Loops](../development/AI-Coding-Loops.md) — places this PRD-progress.txt style loop within the broader coding-loop taxonomy and frames the harness engineering decisions (feedback loops, iteration caps) it makes implicitly.
